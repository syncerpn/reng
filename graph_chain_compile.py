...
def main():
    ...
    save_dir = args.save_dir if args.save_dir else os.path.join(args.meta_path, "graph")

    assert args.skip_existed or not os.path.exists(save_dir), "[info] graph saving path already exists"
    ...
    for i, model_name in enumerate(meta_data):
        graph_file = os.path.join(save_dir, model_name + ".txt")
        if args.skip_existed and os.path.exists(graph_file):
        ...
        command = f"python -m onnx_tool -i {onnx_file} -f {graph_file}"
