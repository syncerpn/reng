...
def get_args():
    ...
    parser.add_argument("--s", type=str, required=True, help="s data header")
...
def main():
    ...
    assert args.s in header, f"[erro] s header not found in the report"
    ...
    y_data = data[args.y]
    s_data = data[args.s]
    s_data = [si * 2 for si in s_data]

    # s_data = [min(si / max(s_data), 20) for si in s_data]

    plt.figure(figsize=(12, 6))
    plt.grid(True, which="both", linestyle="--", linewidth=0.5)
    plt.rc('axes', axisbelow=True)

    plt.scatter(x_data, y_data, color="b", s=s_data, alpha=0.2)
